...
// Obtém o ID do cliente da URL e valida
$idCliente = filter_input(INPUT_GET, 'id', FILTER_SANITIZE_STRING);
if (!$idCliente ||!isValidObjectId($idCliente)) {
...
try {
    ...
    $objectId = new ObjectId($idCliente);
    ...
    // Carrega o DAO do Cliente para buscar os dados
    $clienteDAO = new ClienteDAO($client->Agenda->Clientes); // Correção aqui
    $data = $clienteDAO->read($objectId);


} catch (\Throwable $th) {
    echo "Erro ao buscar cliente: ". $th->getMessage();
}
?>
                ...
                <?php if ($data): ?>
                    ...
                    <div class="control-group container text-center col-md-6 col-sm-6 col-xs-12">
                        ...
                        <div class="controls form-control disabled">
                            <label class="carousel-inner">
                                <?php echo htmlspecialchars($data->getCpf()); ?>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="control-group container text-center col-md-6 col-sm-6 col-xs-12">
                        ...
                        <div class="controls form-check disabled">
                            <label class="carousel-inner container text-center col-md-6 col-sm-6 col-xs-12">
                                <?php echo htmlspecialchars($data->getSexo()); ?>
                            </label>
                        </div>
                    </div>
                    <br/>
                    <div class="text-center">
                        <a href="../../../clientes.php"><button type="button" class="btn btn-primary">Voltar</button></a>
                    </div>
                <?php else: ?>
